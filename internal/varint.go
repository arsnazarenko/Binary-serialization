...
func _serializeUintInto(value uint64, buf []byte) ([]byte, error) {
    ...
    bytesLen, remainder := bitsLen / 7, value % 7
    if remainder > 0 {
        bytesLen++
    }
...
func _deserializeUint(data []byte) (uint64, error) {
    res := uint64(0)
    i := 0
    for {
...
func DeserializeUint(data []byte) (uint64, error) {
    if len(data) < 2 { // tag and one byte is minimal size of varint
        return 0, fmt.Errorf("Invalid row data")
    }
    if data[0] != VarintType {
        return 0, fmt.Errorf("Buffer contains the wrong data type")
    }
    return _deserializeUint(data[1:])
}
...
func DeserializeInt(data []byte) (int64, error) {
    ...
    unsigned, err := _deserializeUint(data[1:])
    if err != nil {
        return 0, err
    }
